...
# 변수는 config.env에서 로드됨 (TPS 2000 전용 변수 사용)
BASE_URL="${BASE_URL}"
SCHEDULE_ID="${SCHEDULE_ID}"
CONCERT_ID="${CONCERT_ID}"
K6_VUS_2000="${K6_VUS_2000:-2000}"
K6_DURATION_2000="${K6_DURATION_2000:-60s}"
TARGET_TPS_2000="${TARGET_TPS_2000:-2000}"
STATUS_POLL_MS_2000="${STATUS_POLL_MS_2000:-1000}"
...
NUM_USERS="$K6_VUS_2000" "$SCRIPT_DIR/02-prepare-tokens.sh"
echo ""
...
echo "k6 실행: VUs=$K6_VUS_2000, Duration=$K6_DURATION_2000, TPS=$TARGET_TPS_2000"
echo ""
...
k6 run \
  ...
  --env TARGET_TPS="$TARGET_TPS_2000" \
  --env STATUS_POLL_MS="$STATUS_POLL_MS_2000" \
  --env HEARTBEAT_INTERVAL_MS="$HEARTBEAT_INTERVAL_MS" \
  --env VUS="$K6_VUS_2000" \
  --env DURATION="$K6_DURATION_2000" \
