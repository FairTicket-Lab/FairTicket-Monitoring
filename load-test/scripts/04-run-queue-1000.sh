...
# 변수는 config.env에서 로드됨 (TPS 1000 전용 변수 사용)
BASE_URL="${BASE_URL}"
SCHEDULE_ID="${SCHEDULE_ID}"
CONCERT_ID="${CONCERT_ID}"
K6_VUS_1000="${K6_VUS_1000:-1000}"
K6_DURATION_1000="${K6_DURATION_1000:-60s}"
TARGET_TPS_1000="${TARGET_TPS_1000:-1000}"
STATUS_POLL_MS_1000="${STATUS_POLL_MS_1000:-1000}"
...
NUM_USERS="$K6_VUS_1000" "$SCRIPT_DIR/02-prepare-tokens.sh"
echo ""
...
echo "k6 실행: VUs=$K6_VUS_1000, Duration=$K6_DURATION_1000, TPS=$TARGET_TPS_1000"
echo ""
...
k6 run \
  ...
  --env TARGET_TPS="$TARGET_TPS_1000" \
  --env STATUS_POLL_MS="$STATUS_POLL_MS_1000" \
  --env HEARTBEAT_INTERVAL_MS="$HEARTBEAT_INTERVAL_MS" \
  --env VUS="$K6_VUS_1000" \
  --env DURATION="$K6_DURATION_1000" \
