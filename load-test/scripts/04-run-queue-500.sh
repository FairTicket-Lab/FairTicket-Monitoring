...
# 변수는 config.env에서 로드됨 (TPS 500 전용 변수 사용)
BASE_URL="${BASE_URL}"
SCHEDULE_ID="${SCHEDULE_ID}"
CONCERT_ID="${CONCERT_ID}"
K6_VUS_500="${K6_VUS_500:-500}"
K6_DURATION_500="${K6_DURATION_500:-60s}"
TARGET_TPS_500="${TARGET_TPS_500:-500}"
STATUS_POLL_MS_500="${STATUS_POLL_MS_500:-1000}"
...
NUM_USERS="$K6_VUS_500" "$SCRIPT_DIR/02-prepare-tokens.sh"
echo ""
...
echo "k6 실행: VUs=$K6_VUS_500, Duration=$K6_DURATION_500, TPS=$TARGET_TPS_500"
echo ""
...
k6 run \
  ...
  --env TARGET_TPS="$TARGET_TPS_500" \
  --env STATUS_POLL_MS="$STATUS_POLL_MS_500" \
  --env HEARTBEAT_INTERVAL_MS="$HEARTBEAT_INTERVAL_MS" \
  --env VUS="$K6_VUS_500" \
  --env DURATION="$K6_DURATION_500" \
